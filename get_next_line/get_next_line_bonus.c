...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024-10-29 18:42:58 by milsanch          #+#    #+#             */
/*   Updated: 2024-10-29 18:42:58 by milsanch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	next_line[fd] = read_line(fd, buff, next_line[fd]);
	free(buff);
	if (!next_line[fd])
		return (NULL);
	line = final_line(next_line[fd]);
	if (!line)
		return (NULL);
	next_line[fd] = clean_nextline(next_line[fd]);
	return (line);
}
